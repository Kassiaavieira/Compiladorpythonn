...
_lr_productions = [
  ...
  ('functions -> function functions','functions',2,'p_func','pparser.py',30),
  ('functions -> empty','functions',1,'p_func','pparser.py',31),
  ('function -> FUNC ID LPAREN params RPAREN LBR statements RBR','function',8,'p_function','pparser.py',42),
  ('pcall -> logic pclist','pcall',2,'p_params_call','pparser.py',48),
  ('pcall -> empty','pcall',1,'p_params_call','pparser.py',49),
  ('pclist -> COMMA logic pclist','pclist',3,'p_pclist','pparser.py',58),
  ('pclist -> empty','pclist',1,'p_pclist','pparser.py',59),
  ('params -> ID plist','params',2,'p_params','pparser.py',67),
  ('params -> empty','params',1,'p_params','pparser.py',68),
  ('plist -> COMMA ID plist','plist',3,'p_plist','pparser.py',80),
  ('plist -> empty','plist',1,'p_plist','pparser.py',81),
  ('statements -> statement statements','statements',2,'p_s','pparser.py',89),
  ('statements -> empty','statements',1,'p_s','pparser.py',90),
  ('statement -> attrib','statement',1,'p_statement','pparser.py',97),
  ('statement -> if','statement',1,'p_statement','pparser.py',98),
  ('statement -> while','statement',1,'p_statement','pparser.py',99),
  ('statement -> return','statement',1,'p_statement','pparser.py',100),
  ('empty -> <empty>','empty',0,'p_empty','pparser.py',105),
  ('if -> IF LPAREN logic RPAREN LBR statements RBR','if',7,'p_if','pparser.py',109),
  ('while -> WHILE LPAREN logic RPAREN LBR statements RBR','while',7,'p_while','pparser.py',118),
  ('return -> RETURN logic SEMICOLON','return',3,'p_return','pparser.py',130),
  ('attrib -> ID ATTRIB logic SEMICOLON','attrib',4,'p_attrib','pparser.py',134),
  ('logic -> logic LT logic','logic',3,'p_logic','pparser.py',144),
  ('logic -> expression','logic',1,'p_logic_expression','pparser.py',148),
  ('expression -> expression PLUS expression','expression',3,'p_e','pparser.py',152),
  ('expression -> expression MINUS expression','expression',3,'p_expression_minus','pparser.py',156),
  ('expression -> term','expression',1,'p_expression_term','pparser.py',161),
  ('term -> term TIMES term','term',3,'p_term','pparser.py',165),
  ('term -> term DIVIDE term','term',3,'p_term_div','pparser.py',171),
  ('term -> call','term',1,'p_term_call','pparser.py',176),
  ('term -> NUMBER','term',1,'p_term_number','pparser.py',180),
  ('term -> ID','term',1,'p_term_id','pparser.py',184),
  ('call -> ID LPAREN pcall RPAREN','call',4,'p_call','pparser.py',194),
  ('term -> LPAREN expression RPAREN','term',3,'p_paren','pparser.py',198),
]
